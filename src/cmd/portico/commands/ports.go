...
func NewPortsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(parentCmd *cobra.Command, args []string) {
			...
			// Find subcommand after "ports"
			for i := portsIndex + 1; i < len(allArgs); i++ {
...
// getServiceNameFromPortsArgs extracts service-name from ports command arguments
// It parses os.Args to find the service-name after "ports" and app-name
func getServiceNameFromPortsArgs(cmd *cobra.Command) (string, error) {
	// Parse os.Args to find service-name after "ports" and app-name
	args := os.Args[1:] // Skip program name
	for i, arg := range args {
		if arg == "ports" {
			// Find app-name first
